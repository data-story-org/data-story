...
export default class NodeWidget extends React.Component {
  ...
  renderInPorts() {
    return Object.values(this.props.node.getInPorts()).map(
      (port) => {
        return (
          <div
          ...
          >
            ...
            <div className="flex w-full items-center text-gray-200 py-1 border border-gray-900 rounded-lg bg-gray-500">
              <div className="flex items-center justify-between w-full">
                ...
                <NodeInspectorLink
                  store={store}
                  nodeId={this.props.node.options.id}
                />
              </div>
            </div>
            <div className="ml-2">
  ...
  renderInspectIcon() {
    return (
      this.props.node.isInspectable() && (
        <div
          onClick={(e) => {
            this.props.store.goToInspector(
  ...
  renderOutPorts() {
    return Object.values(this.props.node.getOutPorts()).map(
      (port) => {
        return (
          <div
            className="flex w-full"
