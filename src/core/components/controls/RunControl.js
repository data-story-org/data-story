...
export default observer(
  class RunControl extends BaseControl {
    constructor(props) {
      super(props);
      this.id = 'run';
    ...
    onClick() {
      ...
      this.props.store.metadata.client
        .run(this.props.store.diagram.engine.model)
        .then((response) => {
          ...
          response.data.diagram.nodes
            ...
            .forEach((phpNode) => {
              let reactNode =
                this.props.store.diagram.engine.model.getNode(
                  phpNode.id,
                );
