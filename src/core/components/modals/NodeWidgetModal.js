...
export default observer(
  class NodeWidgetModal extends React.Component {
    constructor(props) {
      super(props);
          ...
          <div className="flex justify-between">
            <p className="text-sm font-medium text-gray-900 text-bold">
              <span className="text-indigo-500">
                {this.props.node.options.category}
              </span>
              <span className="">
                {' '}
                / {this.props.node.getDisplayName()}
              </span>
            </p>
            <p
      ...
      return (
        <div>
          <div className="w-full bg-gray-100 px-6 py-2">
            {Object.values(this.state.parameters).map(
              (parameter) => {
    ...
    renderEditableInPorts() {
      return (
        this.props.node.options.editableInPorts && (
                ...
                >
                  <div className="w-full rounded">
                    <input
                      ...
                      onChange={this.editExistingPort.bind(
                        this,
                      )}
                    />
                  </div>
    ...
    renderEditableOutPorts() {
      return (
        this.props.node.options.editableOutPorts && (
          <div className="w-full px-6 py-1 text-gray-500 text-xs font-mono border border-t">
            <div className="my-2">Ports</div>
            {Object.values(
